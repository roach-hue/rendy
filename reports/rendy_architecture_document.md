...
팝업스토어 공간 배치는 디자이너가 도면을 보고 수작업으로 기물을 배치하는 과정입니다. 시간이 오래 걸리고, 소방/시공 규정 위반을 사후에 발견하는 경우가 많습니다.

LLM에게 "이 공간에 오브젝트를 배치해줘"라고 시키면? **좌표를 만들어냅니다.** `(3500, 2100)`처럼 그럴듯한 숫자를 출력하지만, 실제로는 벽 안에 박혀있거나, 다른 기물과 겹치거나, 소방 통로를 막습니다. LLM은 기하학을 모릅니다.

### 해결: AI-코드 역할 분리

```
LLM: "이 캐릭터는 입구 쪽에, 벽을 바라보고 놓아주세요"
코드: (좌표 계산) → (충돌 검사) → (통로 확인) → (규정 검증) → 배치 확정
```

LLM은 **"어디에, 어떤 방향으로"**만 결정합니다 (zone_label + direction).  
**좌표, 회전각, 충돌 검증, 통로 확보**는 전부 Shapely/NetworkX 코드가 계산합니다.

LLM이 틀려도 시스템이 깨지지 않습니다:
- 좌표를 출력하면 → Pydantic이 차단
- 임의 각도를 출력하면 → 직교(0/90/180/270)로 snap
- 존재하지 않는 zone을 지정하면 → 인접 zone으로 자동 교정
- 배치가 전부 실패하면 → deterministic fallback이 코드로 강제 배치

### 검증

모든 배치 결과는 5대 소방/시공 규정을 자동 검증합니다:
- 소방 주통로 900mm
- 비상 대피로 1200mm
- Dead Zone (설비 주변) 침범
- 벽체 이격 300mm
...
blocking(배치 무효)과 warning(경고)으로 분류하여 리포트에 명시합니다.
