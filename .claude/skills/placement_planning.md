...
**inward**: placement_slot에서 entrance 방향으로 오프셋
- 오프셋 규칙 미결 — 실제 오브젝트 크기 확인 후 결정

**center**: placement_slot에서 floor center 방향으로 오프셋
- 오프셋 규칙 미결 — 실제 오브젝트 크기 확인 후 결정
...
zone 안의 placement_slot 전체 순회
    └→ calculate_position
    └→ zone_polygon.contains(object_bbox) 선행 체크 → 이탈 시 skip
