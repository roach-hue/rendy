...
각 어댑터는 내부에서 도면 타입을 감지하고 타입별 추출 로직을 실행. 외부 구조 변경 없음.

| 형식 | 도면 타입 감지 방법 |
|------|-------------------|
| DXF | 레이아웃 이름 / 레이어명 ("단면", "SECTION", "S-") |
| PDF | 타이틀 블록 OCR ("단면도", "S-1") |
| 이미지 | Claude Vision |

모든 파서는 `ParsedDrawings` 공통 스키마로 정규화 후 후속 처리에 전달.
...
- 파서가 어떤 형식이든 출력 스키마는 동일 (ParsedDrawings)
