...
| inward 오프셋 규칙 | placement_slot에서 entrance 방향으로 얼마나 띄울지 — 실제 오브젝트 크기 확인 후 결정 |
| center 오프셋 규칙 | placement_slot에서 floor center 방향으로 얼마나 띄울지 — 실제 오브젝트 크기 확인 후 결정 |
| 비정형 공간 zone 임계값 | walk_mm 기반 zone 구분의 임계값 수치 — 실제 도면 테스트 후 결정 |
| 내부 벽/방 구분 | 배치 가능 영역 추출 규칙 + 복수 입구 처리 — 아키텍처 수준 결정 필요 |
| 곡선 벽 근사 오차 | scale_mm_per_px 기반 epsilon 직접 지정으로 확정 (sagitta ≤ 1mm) |
